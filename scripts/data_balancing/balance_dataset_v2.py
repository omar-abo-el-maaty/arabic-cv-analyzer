...
======================
شغّله بـ: python balance_dataset_v2.py

✅ الجديد في v2:
  - Resume تلقائي: لو arabic_cvs_balanced.csv موجود بالفعل، يكمل منه
    (يحسب التوزيع الحالي فيه ويولّد الناقص بس)
  - بيحفظ على OUTPUT_CSV نفسه كل 25 CV (مش ملفات checkpoint منفصلة)
  - Retry على مستوى الـ row كامل (مش بس على مستوى الـ Ollama call)
  - مفيش تكرار رسائل checkpoint للفاشل
"""
...
#  ✏️  عدّل هنا بس
# ══════════════════════════════════════════════════════
INPUT_CSV    = "arabic_cvs_with_scores.csv"   # المصدر الأصلي (يُستخدم مرة واحدة فقط لو OUTPUT غير موجود)
OUTPUT_CSV   = "arabic_cvs_balanced.csv"      # ده اللي بيتم القراءة منه والكتابة عليه (resume)
TARGET       = 400
OLLAMA_MODEL = "qwen2.5:7b"
OLLAMA_URL   = "http://localhost:11434/api/generate"
ROW_RETRIES  = 2          # لو فشل توليد row كامل، يعيد المحاولة كمان كام مرة
SAVE_EVERY   = 25         # يحفظ على OUTPUT_CSV كل كام CV جديد
# ══════════════════════════════════════════════════════
...
MIN_WORDS = 60   # ✅ أقل عدد كلمات مقبول (كان 25 وده كان بيقبل CVs ضعيفة جداً غير واقعية)
...
def _try_generate_once(category_en, level, score):
    """محاولة واحدة لتوليد row — ممكن تفشل"""
...
if __name__ == '__main__':
    ...
    print(f'\n{"="*55}')
    print(f'  Arabic CV Dataset Balancer — v2 (Resume Support)')
    print(f'{"="*55}')
    ...
    print(f'📂 Working file: {output_path if is_resume else input_path}')
    ...
    for lv, (lo, hi) in CLASS_SCORE_RANGES.items():
        ...
        bar  = '█' * int(pct / 2)
        need = max(0, TARGET - n)
        print(f'   {lv:>6} ({lo}-{hi}): {n:>5}  ({pct:5.1f}%)  {bar}  {"→ +"+str(need) if need else "✅ كافي"}')
    ...
    print(f'\n🚀 Starting generation of {total_needed} CVs...\n')

    generated     = 0
    failed        = 0
    start_time    = time.time()
    max_id        = int(df['id'].max()) + 1
    since_save    = 0     # عداد الـ rows من آخر save (يصفّر بعد كل save)
    ...
    for level, count in needed.items():
        ...
        for i in range(count):
            ...
            print(f'  [{done+1:>4}/{total_needed}] {category:<25} score={score}  ', end='', flush=True)
