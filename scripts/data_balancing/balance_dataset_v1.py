...
balance_dataset.py
==================
شغّله بـ: python balance_dataset.py
"""
...
TARGET     = 400          # عدد الـ samples المطلوب لكل class
...
# ── Categories (نفس الموجودة في الـ CSV) ──────────────
CATEGORIES_EN = [
...
def generate_row(category_en, level, score):
...
تعليمات: اكتب باللغة العربية فقط، ابدأ مباشرةً بمحتوى الـ CV بدون أي مقدمة.

اكتب الـ CV الآن:""")

    if not cv_text or len(cv_text.split()) < 25:
        return None
...
def save_checkpoint(df_orig, new_rows, path):
    try:
        new_df = pd.DataFrame(new_rows)[list(df_orig.columns)]
        pd.concat([df_orig, new_df], ignore_index=True).to_csv(
...
if __name__ == '__main__':
    ...
    # Load
    print(f'\n{"="*55}')
    print(f'  Arabic CV Dataset Balancer')
    print(f'{"="*55}')
    print(f'\n📂 Input  : {INPUT_CSV}')
    ...
    for level, count in needed.items():
        ...
        for i in range(count):
            ...
            if new_rows and len(new_rows) % 25 == 0:
                save_checkpoint(df, new_rows, BACKUP_DIR / f'checkpoint_{len(new_rows)}.csv')
    ...
    # Merge & Save
    if not new_rows:
        print('\n❌ Nothing generated. Check Ollama is running.')
        exit()

    new_df   = pd.DataFrame(new_rows)[list(df.columns)]
    df_final = pd.concat([df, new_df], ignore_index=True)

    # New distribution
    new_dist = get_dist(df_final)
    print(f'\n📊 New Distribution:')
    for lv, (lo, hi) in CLASS_SCORE_RANGES.items():
        n_old = cur[lv]
        n_new = new_dist[lv]
        pct   = n_new / len(df_final) * 100
        bar   = '█' * int(pct / 2)
        print(f'   {lv:>6}: {n_old:>4} → {n_new:>4} ({pct:4.1f}%) {bar}')
